...
- `resolution`：`original` 表示跟随摄像头实际输出，也可明确请求 `720p` 或 `1080p`。
- `fps`：`0` 表示跟随摄像头实际输出，也可选择固定的 15、20、24、25、30、50 或 60 FPS。
- `recording_video_quality`：`hd`（高清）、`high`（高质量）、`source`（原画优先），默认 `source`。
- `recording_max_long_edge`：设置为 `0` 表示不限制且不缩放；也可选择 `960`、`1280`、`1920` 主动等比例缩小，绝不会放大低分辨率画面。
- `video_format`：默认 `mp4`。
...
基础配置已移动到右上角“设置”入口中。分辨率、帧率默认跟随摄像头输出，录制画质默认原画优先，录制长边默认不限制。正式视频使用随软件提供的 FFmpeg H.264 编码器；预览缩放不会改变录制视频尺寸。正在录制时不建议修改基础配置，请结束录制后再调整。
