...
- 开始录制、结束录制、切换录制和重复录制时支持系统语音提示；语音提示可开启或关闭，提示文字可自定义。
- 当前版本不支持自定义音频文件，语音提示不会录入视频文件。
...
- `voice_prompt`：录制语音提示配置，支持开启/关闭，并可分别设置开始录制、结束录制、切换录制和重复录制提示语。
...
语音提示使用本地系统语音，不联网，也不会录入视频文件。为了减少延迟，程序会优先使用常驻 `pyttsx3` 语音引擎，并保留 Windows SAPI / PowerShell 兜底。若语音仍有延迟，建议缩短提示文字，并确认已安装 `pyttsx3` 和 `pywin32`。语音不可用时只会写日志，不影响录制。
...
当前版本：`v1.0.0`。软件可以使用 PyInstaller 打包为 onedir 程序目录，再使用 Inno Setup 制作 Windows 安装包。目标电脑不需要安装 Python，不需要安装 SQLite，也不需要部署任何数据库服务。
...
```text
installer\output\PMSystem_Setup_v1.0.0.exe
```
