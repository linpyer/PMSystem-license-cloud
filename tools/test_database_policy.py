...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            expected = fake_local_app_data / "DDREC" / "data" / "ddrec.db"
            ...
            for forbidden in (canonical, PROJECT_ROOT / "ddrec.db"):
                try:
            ...
            packaged_sources = (
                (PROJECT_ROOT / "DDREC.spec").read_text(encoding="utf-8")
                + (PROJECT_ROOT / "installer" / "DDREC.iss").read_text(encoding="utf-8")
            ).lower()
            assert "ddrec.db" not in packaged_sources
    finally:
