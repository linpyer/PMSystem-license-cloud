function Invoke-MockReleaseFlow {
    ...
    $installer=Join-Path $Root 'DDREC-1.3.0-standard-Setup.exe'
    [IO.File]::WriteAllBytes($installer,[Text.Encoding]::UTF8.GetBytes('fake-ddrec-installer-build-82'))
    ...
    $cloudFinal=Join-Path $release '1.3.0-aaaaaaa';Move-Item $cloudStage $cloudFinal
    ...
    $targetDir=Join-Path $download 'stable\standard\1.3.0\82';New-Item -ItemType Directory $targetDir -Force|Out-Null
    $staged=Join-Path $targetDir '.installer.part';Move-Item $part $staged
    $final=Join-Path $targetDir 'DDREC-1.3.0-standard-Setup.exe';Move-Item $staged $final
    ...
    @{status='draft';sha256=$hash;buildNumber=82}|ConvertTo-Json|Set-Content $draftPath -Encoding utf8NoBOM
...
Describe 'DDREC mocked complete release flow' {
    ...
    It 'runs preflight through Published with Published last' {
        ...
        (Test-Path (Join-Path $script:mockRoot 'downloads\stable\standard\1.3.0\82\DDREC-1.3.0-standard-Setup.exe'))|Should Be $true
    }
    It 'an upload disconnect leaves no final file and no Draft or Published' {
        ...
        (Test-Path (Join-Path $script:mockRoot 'downloads\stable\standard\1.3.0\82\DDREC-1.3.0-standard-Setup.exe'))|Should Be $false
