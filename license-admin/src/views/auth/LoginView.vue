...
<template>
  <main class="auth-page"><section class="auth-panel"><div class="auth-brand"><div class="mark">DD</div><div><h1>DD Rec 授权管理</h1><p>电商影像溯源 · 管理员安全登录</p></div></div><el-form label-position="top" @submit.prevent="submit"><el-form-item label="用户名"><el-input v-model="form.username" autocomplete="username" /></el-form-item><el-form-item label="密码"><el-input v-model="form.password" type="password" show-password autocomplete="current-password" @keyup.enter="submit" /></el-form-item><el-button type="primary" :loading="loading" class="submit" @click="submit">继续验证</el-button></el-form><p class="security">登录后仍需输入动态验证码。连续失败将临时锁定账号。</p></section></main>
</template>
