...
test('管理员登录页面可提交密码',async({page})=>{await page.unroute('**/api/v1/**');await mockApi(page,false);await page.goto('login');await page.getByLabel('用户名').fill('owner');await page.getByLabel('密码').fill('StrongAdmin!2026');await page.getByRole('button',{name:'继续验证'}).click();await expect(page).toHaveURL(/\/admin\/totp/)})
test('TOTP验证进入首页',async({page})=>{await page.unroute('**/api/v1/**');await mockApi(page,false);await page.goto('login');await page.getByLabel('用户名').fill('owner');await page.getByLabel('密码').fill('x');await page.getByRole('button',{name:'继续验证'}).click();await expect(page).toHaveURL(/\/admin\/totp/);await page.locator('.code input').fill('123456');await page.getByRole('button',{name:'登录管理后台'}).click();await expect(page.getByRole('heading',{name:'首页概览'})).toBeVisible()})
